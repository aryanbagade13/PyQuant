...
def black_scholes_price(
    ...
    if volatility == 0:
        discounted_spot = spot * math.exp(-dividend_yield * time_to_expiry_value)

        discounted_strike = strike * math.exp(-risk_free_rate * time_to_expiry_value)
    ...
    if option.option_type == "call":
        return spot * math.exp(-dividend_yield * time_to_expiry_value) * normal_cdf(
            d1
        ) - strike * math.exp(-risk_free_rate * time_to_expiry_value) * normal_cdf(d2)

    return strike * math.exp(-risk_free_rate * time_to_expiry_value) * normal_cdf(
        -d2
    ) - spot * math.exp(-dividend_yield * time_to_expiry_value) * normal_cdf(-d1)
